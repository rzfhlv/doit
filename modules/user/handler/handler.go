...
func (h *Handler) Register(e echo.Context) (err error) {
	...
	if err != nil {
		...
		return e.JSON(http.StatusBadRequest, utilities.SetResponse(message.ERROR, err.Error(), nil, nil))
	}
	...
	return e.JSON(http.StatusOK, utilities.SetResponse(message.OK, message.SUCCESS, nil, result))
}
...
func (h *Handler) Login(e echo.Context) (err error) {
	...
	if err != nil {
		if err == sql.ErrNoRows || err == bcrypt.ErrMismatchedHashAndPassword {
			...
			return e.JSON(http.StatusBadRequest, utilities.SetResponse(message.ERROR, message.SOMETHINGWENTWRONG, nil, nil))
		}
		logrus.Log(nil).Error(fmt.Sprintf("User Handler Login Usecase, %v", err.Error()))
		return e.JSON(http.StatusInternalServerError, utilities.SetResponse(message.ERROR, message.SOMETHINGWENTWRONG, nil, nil))
	}
...
func (h *Handler) Logout(e echo.Context) (err error) {
	...
	if len(split) < 2 {
		...
		return e.JSON(http.StatusUnauthorized, utilities.SetResponse(message.ERROR, message.UNAUTHORIZED, nil, nil))
	}
